...
def render():
    ...
    st.markdown(f'<span style="{_TTL}">Can a model predict who scores, and what does it lean on?</span>', unsafe_allow_html=True)
    st.markdown(
    ...
    with cols[0]:
        ...
        st.markdown(
            '<div style="font-size:.85rem;color:#9A9AA8;margin-top:.3rem;font-family:\'Titillium Web\',sans-serif;">'
            'Each rung adds one more layer of information. The model never saw the future while learning.'
            '</div>',
            unsafe_allow_html=True,
        )
    with cols[1]:
        ...
        st.markdown(
            ...
            f'<div class="stat-label">Strategy lift (M2 vs M1)</div>'
    ...
    # Two-slice donut: Car quality vs all other features (honest framing)
    all_other = round(1.0 - b4["car_quality_share"], 4)
    fig = go.Figure(go.Pie(
        labels=["Car quality", "All other features"],
        values=[b4["car_quality_share"], all_other],
        hole=.55,
        marker_colors=["#E10600", "#3A3A50"],
    ...
    fig.update_layout(
        ...
        annotations=[dict(
            text=f'<b>{b4["car_quality_share"]*100:.1f}%</b><br><span style="font-size:11px">Car quality</span>',
            x=0.5, y=0.5, font_size=18, showarrow=False,
        )],
    )

    donut_col, alpha_col = st.columns([2, 1])
    with donut_col:
        st.plotly_chart(fig, use_container_width=True)
    with alpha_col:
        st.markdown(
            f'<div class="card reveal" style="margin-top:1rem">'
            f'<div style="font-family:JetBrains Mono,monospace;font-weight:700;font-size:2.2rem;color:#00D2BE;line-height:1">'
            f'{b4["strategic_alpha"]*100:.1f}%</div>'
            f'<div class="stat-label">Strategy + context</div>'
            f'<div class="stat-caption">'
            f'Broader upper bound estimate from the final season fold (XGBoost, 2024 test set, where strategy data is fully observed). '
            f'A more generous measure than the strategy feature group alone (6.4%). '
            f'Roughly one sixth of what the model can anticipate, at most, comes from strategy and the context around it.'
            f'</div>'
            f'</div>',
            unsafe_allow_html=True,
        )
